...
class NtfyNotifier:
    TITLES = dict(guard_pause='Scanner paused', recovery_failed='Recovery stopped',
                  save_failed='Frame save failed', test='Test notification')
    ...
    def notify(self, kind, frame, reason, *, folder='', run_id=None, test_settings=None):
        ...
        key = (run_id, folder, kind, None if kind == 'save_failed' else frame)
        with self.lock:
            if kind != 'test' and key in self.seen:
                return False
            try:
                self.pending.put_nowait((settings, kind, title, message[:3000]))
            except queue.Full:
    ...
    def _run(self):
        while not self.stopping.is_set():
            try:
                ...
                continue
            try:
                settings, kind, title, message = job
                if kind == 'test' or self.get_settings()['enabled']:
                    self._deliver(settings, kind, title, message)
            except Exception:
    ...
    def _deliver(self, settings, kind, title, message):
        payload = dict(topic=settings['topic'], title=f'ALT-Scann8: {title}', message=message,
                       priority=3 if kind == 'test' else 4, tags=['test_tube' if kind == 'test' else 'warning'])
